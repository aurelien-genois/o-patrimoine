...
<section class="home-banner mb-4">
    <?php if (is_array($slides) && !empty($slides)): ?>
        <div class="glide slider-homepage" data-autoplay='5000'>
                    ...
                    <?php foreach ($slides as $slide): ?>
                        <li class="glide__slide page-banner relative -z-40 py-8">
                            ...
                            <div class="container__inner px-4 z-0">
                                ...
                                <?php if ($slide['link']):
                                    ?>
                                    <a href="<?= $slide['link']['url'] ?>" class="btn" alt="<?= $slide['link']['title'] ?>"
                                        title="<?= $slide['link']['title'] ?>" aria-label="<?= $slide['link']['title'] ?>"><?= $slide['link']['title'] ?></a>
                                <?php endif; ?>
                            </div>
            ...
            <div class="glide__bullets absolute bottom-[8px] left-0 right-0 max-w-max mx-auto"
                data-glide-el="controls[nav]">
                <?php foreach ($slides as $key => $slide): ?>
                    <button class="glide__bullet  w-[12px] h-[12px] bg-white rounded-full border border-greyLight border-solid"
                        data-glide-dir="=<?= $key ?>"></button>
                <?php endforeach; ?>
            </div>
        </div>
    <?php endif; ?>
